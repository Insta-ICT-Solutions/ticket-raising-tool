  ...
  //         fetch("https://trt-api-1x35dd1hz-sakgis-projects.vercel.app/admin/create", {
  //           method: "POST",
  //           headers: {
...
function NewTicket({ role }) {  
  ...
  //     const ticketsRef = collection(db, 'users', userId, 'TicketList');
  //     const latestTicketDoc = await getDocs(query(ticketsRef, orderBy('ticketID', 'desc'), limit(1)));
  ...
  //     const ticketId = await generateTicketId(employeeDocId || userId);
  ...
  //     // Use setDoc with a custom ID (ticketID) to ensure the correct ticketID
